...
public class EditLabelNoteCommand implements CommandWithText {

    private final MessageSender messageSender;
    private final RestServiceInterface restService;
    private final DataChatId<Note> noteDataChatId = new DataChatId<>();
    public EditLabelNoteCommand(@Lazy MessageSender messageSender, RestServiceInterface restService) {
    ...
    public void executeWithText(Update update, CommandHandler context) {
        ...
        if (!response.isSuccess()) {
            ...
            answer = "Название \"" + response.getBody().getLabel() + "\" успешно изменено";
        }
